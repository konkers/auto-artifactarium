...
use crate::unk_util::{Achievement, matches_achievement_all_data_notify, matches_get_player_token_rsp, Artifact, matches_artifact_all_data_notify};
...
pub fn matches_artifact_packet(game_command: &GameCommand) -> Option<Vec<Artifact>> {
    return matches_artifact_all_data_notify(game_command.proto_data.clone())
}